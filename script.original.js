...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ─── Lazy-load contact video when section scrolls into view ──────────────
  (function lazyContactVideo() {
    var contactVideo = document.querySelector('.contact-video[data-lazy-src]');
    if (!contactVideo) return;
    var lazySrc = contactVideo.getAttribute('data-lazy-src');
    if (!lazySrc) return;
    var loaded = false;
    var observer = new IntersectionObserver(function(entries) {
      entries.forEach(function(entry) {
        if (entry.isIntersecting && !loaded) {
          ...
          contactVideo.appendChild(source);
          contactVideo.load();
  ...
  // ─── On-demand Plyr loader ───────────────────────────────────────────────
  (function setupPlyrLoader() {
    ...
    window.__loadPlyr = function(callback) {
      ...
      if (callback) plyrCallbacks.push(callback);
      if (plyrLoaded) return; // already loading
      plyrLoaded = true;
      ...
      script.onload = function() {
        plyrCallbacks.forEach(function(cb) { cb(); });
        plyrCallbacks = [];
      };
  ...
  // ─── Mobile hero video source swap ───────────────────────────────────────
  (function mobileHeroSwap() {
    if (window.innerWidth > 768) return;
    var heroVideo = document.querySelector('.hero-video');
    if (!heroVideo) return;
    var source = heroVideo.querySelector('source');
    if (source) {
      source.src = 'assets/videos/HeroSections/Vertical_Sun_girl.mp4';
      heroVideo.poster = 'assets/images/poster_hero_mobile.webp';
      heroVideo.load();
    }
  })();
  ...
  (function setupVideoModal() {
    ...
    function openModal(videoSrc, itemElement) {
      ...
      // Загружаем Plyr на лету при первом открытии модалки
      if (!player && typeof Plyr === "undefined" && window.__loadPlyr) {
        window.__loadPlyr(function() {
